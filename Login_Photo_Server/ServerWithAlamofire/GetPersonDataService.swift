...
struct GetPersonDataService {
    ...
    // -> getPersonInfo 함수가 종료되든 말든 상관없이 completion은 탈출 클로저이기 때문에 , 전달된다면 이후에 외부에서도 사용 가능
    // ** 해당 completion클로저에는 네트워크의 결과를 담아서 호출하게 되고, VC에서 떠내서 처리할 예정
    ...
    func getPersonInfo(completion : @escaping (NetworkResult<Any>) -> Void) {
        ...
        //Request를 보내기 위한 정보를 묶아서 dataRequest에 저장해둔다
        let dataRequest = AF.request(URL, method: .get, headers: header)
        
        //위에서 적어둔 요청서를 가지고 진짜 서버에 보내서 통신 Request를 하느 ㄴ중
        ...
        dataRequest.responseData { dataResponse in
            ...
            //dataResonse가 성동이면 statusCode와 response(결과 데이터)를 guard let 구문을 통해서 저장해둔다
            case .success:
    ...
    //아까 받은 statusCode를 바탕으로 어떻게 결과값을 처리할 건지 정의
    private func judgeStatus(by statusCode: Int, _ data: Data) -> NetworkResult<Any> {
        
        switch statusCode {
        case 200:
            return isValidData(data: data) //성공 - > 데이터를 가공해서 전달해줘야 하기 때문에 isValidDate라는 함수로 데이터 넘겨줌
        case 400:
            return .pathErr // -> 요청이 잘못됨
        case 500:
            return .serverErr // -> 서버 에러
        default:
            return .networkFail // -> 네트워크 에러로 분기 처리할 예정
        }
    }
    
    // 200대로 떨어졌을 대 데이터를 가공하기 위한 함수
    private func isValidData(data: Data) -> NetworkResult<Any> {
        
        //JSON 데이터를 해독하기 위해 JSONDecoder()를 하나 선언
        let decoder = JSONDecoder()
        
        //data를 우리가 만들어둔 PersonDataModel 형으로 decode해준다.
        //실패하면 pathErr로 빼고, 성공하면 decodedData에 값을 뺀다
        guard let decodedData = try? decoder.decode(PersonDataModel.self, from: data) else {
            return .pathErr
        }
        
        //성공적으로 decode를 마치면 success에다가 data 부분을 담아서 completion을 호출함
